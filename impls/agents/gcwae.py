...
class GCWAEAgent(flax.struct.PyTreeNode):
    """Goal-conditioned waypoint advantage estimation (GCWAE) agent."""
    ...
    def actor_loss(self, batch, grad_params):
        """Compute the AWR actor loss, but using the advantage w.r.t. to generated subgoals by the high actor."""
        subgoal_dist = self.network.select('high_actor')(batch['observations'], batch['high_actor_goals'])
        actor_subgoals = subgoal_dist.mode()
        # Use the subgoal advantage as an estimator for the advantage w.r.t. the true goal A(s, a, w).
        v1, v2 = self.network.select('value')(batch['observations'], actor_subgoals)
        nv1, nv2 = self.network.select('value')(batch['next_observations'], actor_subgoals)
        v = (v1 + v2) / 2
        nv = (nv1 + nv2) / 2
        adv = nv - v
    ...
    def high_actor_loss(self, batch, grad_params):
        ...
        if self.config['use_subgoal_rep']:
    ...
    def total_loss(self, batch, grad_params, rng=None):
        ...
        low_actor_loss, low_actor_info = self.actor_loss(batch, grad_params)
    ...
    def sample_actions(
        ...
        if self.config['use_subgoal_rep']:
            ...
            low_dist = self.network.select('low_actor')(observations, goal_reps, goal_encoded=True,
                                                        temperature=temperature)
        else:
            goal = high_dist.sample(seed=high_seed)
            low_dist = self.network.select('low_actor')(observations, goal, temperature=temperature)
    ...
    def create(
        ...
        #  this will be shared between the value and low_actor networks

        if config['encoder'] is not None:
            encoder_module = encoder_modules[config['encoder']]

        if config['use_subgoal_rep']:
            if config['encoder'] is not None:
        ...
        if config['encoder'] is not None:
            ...
            if config['use_subgoal_rep']:
                ...
                encoders['low_actor'] = GCEncoder(state_encoder=encoder_module(), concat_encoder=goal_rep_def)
            else:
                encoders['value'] = GCEncoder(concat_encoder=encoder_module())
                encoders['target_value'] = GCEncoder(concat_encoder=encoder_module())
                encoders['low_actor'] = GCEncoder(concat_encoder=encoder_module())
            encoders['high_actor'] = GCEncoder(concat_encoder=encoder_module())
        else:
            if config['use_subgoal_rep']:
                ...
                encoders['low_actor'] = GCEncoder(state_encoder=Identity(), concat_encoder=goal_rep_def)
                encoders['high_actor'] = None
        ...
        high_actor_def = GCActor(
            hidden_dims=config['actor_hidden_dims'],
            action_dim=config['rep_dim'] if config['use_subgoal_rep'] else ex_observations.shape[-1],
            state_dependent_std=False,
            const_std=config['const_std'],
...
def get_config():
    config = ml_collections.ConfigDict(
        dict(
            ...
            use_subgoal_rep=False,  # Whether to use subgoal representations.
            rep_dim=10,  # Goal representation dimension.
            actor_rep_grad=False,  # Whether low-actor gradients flow to goal representation (use True for pixels).
